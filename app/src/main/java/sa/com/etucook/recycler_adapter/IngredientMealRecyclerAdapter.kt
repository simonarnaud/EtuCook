...
class IngredientMealRecyclerAdapter(private val listener: OnItemClickListener): ListAdapter<Ingredient, IngredientMealRecyclerAdapter.RecyclerViewHolder>(IngredientDiff()) {

    interface OnItemClickListener {
        fun onItemClick(ingredientName: String)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = RecyclerViewHolder(sa.com.etucook.databinding.IngredientItemListBinding.inflate(LayoutInflater.from(parent.context)), listener)
    ...
    class RecyclerViewHolder(private val binding: IngredientItemListBinding, listener: OnItemClickListener): RecyclerView.ViewHolder(binding.root) {
        ...
        init {
            itemView.setOnClickListener { binding.ingredient?.let { listener.onItemClick(it.ingredientName)} }
        }
